...
Three independent status checks are intended to gate pull requests:
...
Configure these checks in branch rules when the repository's GitHub plan permits it.
The current private-repository plan does not support the required branch rules;
maintainers must inspect the checks before merging. Publication enforces its own checks.
...
Badges in README reflect real workflow results for main. Private-repository badges are
not externally shareable; a green badge only describes the configured checks at that run.
...
GitHub CodeQL scanning of private repositories requires the appropriate paid security
feature. New advisories can make a previously green dependency scan fail; review the
